...
class _WeatherCenterProState extends State<WeatherCenterPro> {
  ...
  bool _loading = false;  
  // Suggested Crews calculator â€” wind-driven with severity and window boosts.
        ...
        actions: [
          ...
          Padding(
            ...
            child: FilledButton(
              ...
              child: Text(_loading ? "Runningâ€¦" : "Run Report", style: const TextStyle(fontWeight: FontWeight.w800)),
            ),
          ),
        ],
      ),
          ...
          Expanded(
            child: Column(
              ...
              children: [
                ...
                SizedBox(height: 2),
                Text("Gust:  mph Â· Sustained:  mph", style: const TextStyle(color: kMuted, fontSize: 12)),
              ],
            ),
          ),
          Padding(
            ...
            child: Column(
              ...
              children: [
                ...
                Text(' crews', style: const TextStyle(color: Colors.white70)),
              ],
            ),
          ),
        ],
...
class _ReportRow {
  ...
  final int severity;
  final int crewRec;
// COMPILE-SHIM: out-of-scope UI commented   _ReportRow(this.id, this.name, this.gust, this.sustained, this.severity, this.crewRec);
}


// CREW-SLOT
int _crewFor(num gust, num sustained, int severity, int windowHours) {
  // Simple deterministic calculator, adjust as you like
  final double g = (gust is num) ? gust.toDouble() : 0.0;
  final double s = (sustained is num) ? sustained.toDouble() : 0.0;
  final int sev = severity;
  final int base = (g.round() + (s * 0.6).round()) + (sev * 3);
  final int scale = (windowHours >= 24) ? 1 : 1;
  final int crews = (((base * scale) ~/ 5) < 0 ? 0 : ((base * scale) ~/ 5));
  return crews;
}
